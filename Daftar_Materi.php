...
<body background = "assets/img/bg1.png">
  ...
  <div class="content">
    <h1 class="judul">Daftar Materi</h1>
    <?php
        $no=1;
        $namaMateri=mysqli_query($kon,"SELECT * FROM materi ORDER BY kode_materi ASC");
        while($dataMateri=mysqli_fetch_assoc($namaMateri))
        {
          echo "<div class=\"dropdown tombol m-50\">";
          echo "<button class=\"btn btn-light btn-outline-primary dropdown-toggle btn-sm btn-block\" style=\"text-align:justify; margin-bottom:3%;\" type=\"button\" id=\"dropdownMenuButton\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\">";
          echo $no++." | ".$dataMateri['nama_materi'];
          echo "</button>";
          echo "<div class=\"dropdown-menu\" aria-labelledby=\"dropdownMenuButton\">";

          $subMateri=mysqli_query($kon,"SELECT * FROM submateri ORDER BY kode_submateri ASC");
          while($dataSubMateri=mysqli_fetch_assoc($subMateri))
          {
            if($dataMateri['kode_materi']==substr($dataSubMateri['kode_submateri'],0,3))
            {
              $temp = $dataSubMateri['kode_submateri'];
              echo "<a class=\"btn btn-light btn-outline-primary\" href=\"Penjelasan.php?id=$temp\">";
              echo $dataSubMateri['nama_submateri'];
              echo "</a>";
            }
          }
          echo "</div>";
