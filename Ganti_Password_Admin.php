      ...
      if(strlen($_POST['newPassword'])){
        if (preg_match('~[0-9]+~', $_POST['newPassword']) && (preg_match('/[A-Z]/', $_POST['newPassword']) && (preg_match('/[a-z]/', $_POST['newPassword'])))) {
          if($_POST['newPassword']==$_POST['confirmNewPassword']){
            if(password_verify($_POST['oldPassword'],$data['password_admin'])){
              $newPassword = password_hash($_POST['newPassword'],PASSWORD_DEFAULT);
              $insert = mysqli_query($kon,"UPDATE `admin` SET `password_admin`='$newPassword' WHERE `id_admin`='".$data['id_admin']."'");
              if($insert){ 
                $status = 'success'; 
...
?>
    ...
    <center><form action="Kelola_Profil_Admin.php"><button class="btn my-2 my-sm-0" type="submit" style="background-color : navy; color : white;font-family:arial;">Kembali</button></form></center>
  </div>
</div>
