...
<h1 class="judul">Daftar Kuis</h1>
<?php
        $no=1;
        $namaKuis=mysqli_query($kon,"SELECT * FROM kuis ORDER BY id_kuis ASC");
        while($dataKuis=mysqli_fetch_assoc($namaKuis))
          {
            $temp = $dataKuis['id_kuis'];
            echo "<div class=\"btn-group\" role=\"group\" aria-label=\"Basic example\" style=\"width:80%; margin:1%;\">";
            echo "<a href=\"Kuis.php?id=$temp\" class=\"btn btn-light btn-lg btn-block btn-lg active text-justify\" role=\"button\" aria-pressed=\"true\">";
            echo $no++." | ".$dataKuis['nama_kuis'];
            echo "</a>";
            echo "</div>";
            echo"</br>";
          }
...
<center><form action="Menu.php"><button class="btn my-2 my-sm-0" type="submit" style="background-color : navy; color : white;font-family:arial;">Kembali</button></form></center><br></br>
</div>
